import telebot;
bot = telebot.TeleBot('6425850993:AAEhS2bEQfdqIok5Vfbv811reb3mnPYNkgw')
...
def get_text_massages(message):
    ...
    # Small talk
        ...
        bot.send_message(message.from_user.id, "Спасибо, что интересуешься! В моем случае «дела» можно оценить как «хорошо». Я здесь, чтобы помочь тебе выбрать интересное место для посещения в городе. Расскажи мне, какие типы мест тебя интересуют, и я постараюсь предложить наиболее подходящие варианты.")

    elif message.text == "как дела?":
        bot.send_message(message.from_user.id, "Спасибо, что интересуешься! В моем случае «дела» можно оценить как «хорошо». Я здесь, чтобы помочь тебе выбрать интересное место для посещения в городе. Расскажи мне, какие типы мест тебя интересуют, и я постараюсь предложить наиболее подходящие варианты.")

    elif message.text == "Что делаешь?":
        bot.send_message(message.from_user.id, "Я здесь, чтобы помочь тебе выбрать место, куда сходить в городе. Я могу предложить различные варианты ресторанов, кафе, музеев, парков и других мест, исходя из твоих предпочтений. Расскажи мне, какие типы мест тебя интересуют, и я постараюсь предложить наиболее подходящие варианты.")

    elif message.text == "что делаешь?":
        bot.send_message(message.from_user.id, "Я здесь, чтобы помочь тебе выбрать место, куда сходить в городе. Я могу предложить различные варианты ресторанов, кафе, музеев, парков и других мест, исходя из твоих предпочтений. Расскажи мне, какие типы мест тебя интересуют, и я постараюсь предложить наиболее подходящие варианты.")

    elif message.text == "Зачем тебя создали?":
        bot.send_message(message.from_user.id, "Я был создан для того, чтобы помочь людям в выборе мест, куда можно сходить в городе. Моя цель - предоставить рекомендации и сократить время, которое ты тратишь на поиск подходящих мест. Я учитываю твои предпочтения, предлагаю различные варианты и помогаю сделать выбор. Таким образом, я стараюсь сделать процесс планирования свободного времени более удобным и приятным для тебя.")

    elif message.text == "зачем тебя создали?":
        bot.send_message(message.from_user.id, "Я был создан для того, чтобы помочь людям в выборе мест, куда можно сходить в городе. Моя цель - предоставить рекомендации и сократить время, которое ты тратишь на поиск подходящих мест. Я учитываю твои предпочтения, предлагаю различные варианты и помогаю сделать выбор. Таким образом, я стараюсь сделать процесс планирования свободного времени более удобным и приятным для тебя.")

    elif message.text == "Кто тебя создал?":
        bot.send_message(message.from_user.id, "Я был создан командой разработчиков из НИУ ВШЭ в Нижнем Новгороде. Моя цель — помочь людям найти интересные места для посещения в городе. Если ты не знаешь, куда сходить в Нижнем Новгороде, обращайся ко мне!")

    elif message.text == "кто тебя создал?":
        bot.send_message(message.from_user.id, "Я был создан командой разработчиков из НИУ ВШЭ в Нижнем Новгороде. Моя цель — помочь людям найти интересные места для посещения в городе. Если ты не знаешь, куда сходить в Нижнем Новгороде, обращайся ко мне!")

    elif message.text == "Пока!":
        bot.send_message(message.from_user.id, "До свидания! Если у тебя возникнут вопросы или понадобится помощь в будущем, не стесняйся обращаться. Желаю тебе отличного времени и интересных приключений в городе! Если у тебя есть еще вопросы или нужна помощь в выборе места, обращайся ко мне в любое время.")
